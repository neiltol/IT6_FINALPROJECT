...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $reportQuery = "SELECT 
                        ...
                        MAX(r.renew_date) AS last_renew_date, 
                        s.currency
                    FROM renew r
                    JOIN members m ON r.member_id = m.id
                    LEFT JOIN settings s ON s.id = 1
                    WHERE r.renew_date BETWEEN '$fromDate' AND '$toDate'
                    GROUP BY m.id, m.fullname, m.membership_number, s.currency";
...
<div class="wrapper">
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                  if ($reportResult && $reportResult->num_rows > 0) {
                    ...
                    while ($row = $reportResult->fetch_assoc()) {
                      ...
                      echo '<td>' . $row['currency'] . number_format($row['total_amount'], 2) . '</td>';
                      echo '<td>' . $row['last_renew_date'] . '</td>';
                      echo '</tr>';
                    }
                    ...
                    echo '<div class="card-body">';
                    echo '<p>No revenue records found within the specified date range.</p>';
                    echo '</div>';
                  }
                }
              ?>
  ...
  <footer class="main-footer">
    <strong>&copy; <?php echo date('Y'); ?> Neil</strong> - All rights reserved.
